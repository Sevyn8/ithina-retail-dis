...
- `TODO` Slice 16c: translation + persistence behind the 16a contract. Translate `{src_key, dest_key, declarations}` + `template_type` into the stored D49 `mapping_rules`, run the semantic gate (catalog membership, mandatory coverage, target legality, presence pairings — deferred from 16a), and write the DRAFT row — restoring the create-persistence assertions skip-marked in 16a.
