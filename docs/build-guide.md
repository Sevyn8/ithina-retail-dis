...
- `TODO` Slice 16b: new mapping engine ops the redesigned contract implies but the engine does not yet implement (e.g. `parse_percent` for `src_is_percentage`). A bounded-vocabulary extension in `dis-mapping` (impl + registry + `NORMALIZE_OPS` + arg validator, kept in sync by the parity drift test), not a per-source code path.
